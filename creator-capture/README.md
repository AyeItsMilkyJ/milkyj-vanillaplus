...
This directory is a fail-closed registry for creator-side candidates evaluated for Mundane Craft. It contains no mod JARs, no Packwiz metafiles and no installer. Both currently evaluated candidates are ineligible, so ordinary players and the dedicated server receive no creator tooling.
