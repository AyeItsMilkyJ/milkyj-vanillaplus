...
$webhookProcess = Start-Process -FilePath $python -ArgumentList @(
    ('"' + $fakeServer + '"'), '--port', "$port", '--output', ('"' + $recording + '"')
...
try {
    ...
    $events = @('test', 'starting', 'online', 'restarting', 'crashed', 'warning', 'offline', 'failed')
    ...
    } while ($records.Count -lt $events.Count -and (Get-Date) -lt $deadline)
    if ($records.Count -ne $events.Count) { throw "Expected $($events.Count) webhook requests; recorded $($records.Count)." }
    ...
    if (@(Compare-Object $events $recordedEvents).Count -ne 0) {
        throw "Webhook event mismatch. Expected=$($events -join ','); actual=$($recordedEvents -join ',')"
    }
