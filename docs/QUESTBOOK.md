...
The current candidate is `1.9.0-rc2` for Minecraft `1.20.1` and Forge `47.4.10`. The established previous version is `1.8.1-packwiz.1`; the local `v1.0.0` Git label is a recovery baseline, not the pack's semantic version. The candidate remains test-only until all required manual checks pass.
