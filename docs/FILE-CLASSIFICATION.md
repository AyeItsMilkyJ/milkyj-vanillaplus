...
The Packwiz index contains 724 unique destinations in total:
...
Across all 724 destinations, 526 are `both`, 183 are `client`, and 15 are `server`. Internal files are represented by side-aware `.pw.toml` metadata and repository-hosted payloads, rather than unconditionally copied internal files. The count reflects the 15-chapter guide, current compatibility datapack, curated resource-pack defaults, and exact client/server classification.
