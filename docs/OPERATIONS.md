...
The maintained quest source is generated by `scripts\Build-BeginnerQuestBook.ps1`. The script reads the preserved 1.8.0 definitions under `audit\questbook-legacy-1.8.0`, loads the main progression from `scripts\quest-content\ProgressionExpansion.ps1` and the 24 optional RC4 leaves from `scripts\quest-content\ComedyExpansion.ps1`, retains every selected published quest/task/reward ID, validates the 15-chapter/234-quest graph, and then refreshes the hosted Packwiz payload.
...
Double-click root `run.bat` for Minecraft's real graphical server window. The brief launcher terminal closes automatically; a hidden PowerShell supervisor keeps ownership of Java, scheduled restarts and crash recovery. Enter ordinary Minecraft commands in the GUI's bottom field. Use `STOP SERVER.bat` for the safest full save/shutdown and `RESTART SERVER.bat` for a controlled restart. `RUN SERVER CONSOLE.bat` is the separate raw-terminal troubleshooting mode. The `packwiz-tools` folder also contains `START SERVER.bat`, `STOP SERVER.bat`, `RESTART SERVER.bat`, `SERVER STATUS.bat`, `BACKUP SERVER.bat`, and operator-controlled `UPDATE SERVER.bat`. The supervisor restarts cleanly every 180 minutes after `Done`, recovers unexpected crashes with bounded backoff, and never applies an update by itself.
...
2. open the quest book and verify `START HERE, YOU BEAUTIFUL NOOB` is immediately visible;
