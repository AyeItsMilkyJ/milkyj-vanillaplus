...
The maintained quest source is generated by `scripts\Build-BeginnerQuestBook.ps1`. The script reads the preserved 1.8.0 definitions under `audit\questbook-legacy-1.8.0`, loads the authored expansion from `scripts\quest-content\ProgressionExpansion.ps1`, retains selected quest/task/reward IDs, validates the 15-chapter/210-quest graph, and then refreshes the hosted Packwiz payload.
